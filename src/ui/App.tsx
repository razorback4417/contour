...
import { topologyInsights } from "./insights";

const filterKinds: NodeKind[] = ["cpu_package", "cpu_core", "cache", "numa_node", "memory_region", "pci_bridge", "pci_endpoint", "gpu", "nic", "rdma_device", "network_port", "network_interface", "storage_device"];
...
export function App() {
  ...
  const tracedPath = useMemo(() => selected.length === 2 ? traceTopologyPath(selected[0], selected[1], nodeById) : [], [selected, nodeById]);
  const highlighted = useMemo(() => {
    ...
    return ids;
  }, [selected, numa, snapshot, tracedPath]);
  ...
  useEffect(() => {
    ...
    fetch("/api/snapshot", { headers: { accept: "application/json" } }).then(async (response) => {
      ...
      if (!cancelled) resetSnapshot(next);
    }).catch((reason) => {
  ...
  function exportSvg() {
    ...
    download(svg, "image/svg+xml", `${nodeById.get(snapshot.hostId)?.label ?? "topology"}.svg`);
  }
  ...
  return <div className="shell">
    <header><div><span className="wordmark">CONTOUR</span><span className="subtitle">SYSTEM TOPOLOGY EXPLORER</span></div><div className="header-actions">
      ...
      <label className="button">Load snapshot<input type="file" accept=".xml,.json,text/xml,application/json" onChange={openFile}/></label>
      <details className="export-menu"><summary>Export</summary><div><button onClick={exportSnapshot}>Snapshot JSON</button><button onClick={exportSvg}>Diagram SVG</button></div></details>
    </div></header>
    ...
    <main>
      <aside className="controls">
        ...
        <section className="legend"><label className="section-label">RELATIONSHIPS</label><div><i className="contains"/><span><b>contains</b> source hierarchy</span></div><div><i className="backed"/><span><b>backed by</b> OS → PCI device</span></div><div><i className="exposes"/><span><b>exposes</b> device → port</span></div><div><i className="connected"/><span><b>connected to</b> RDMA port ↔ netdev</span></div><div><i className="local"/><span><b>local to</b> explicit NUMA evidence</span></div><p>Edges show known topology facts—not measured traffic or bandwidth.</p></section>
      </aside>
      <section className="viewport">
        ...
        <div className="trace-state"><b>PATH TRACE</b>{selected.length === 0 && <span>Select endpoint A</span>}{selected.length === 1 && <span>A · {truncate(nodeById.get(selected[0])?.label ?? selected[0], 24)} → select B</span>}{selected.length === 2 && <span>A · {truncate(nodeById.get(selected[0])?.label ?? selected[0], 16)} → B · {truncate(nodeById.get(selected[1])?.label ?? selected[1], 16)} · {Math.max(0, tracedPath.length - 1)} hops</span>}{selected.length > 0 && <button onClick={() => setSelected([])}>CLEAR</button>}</div>
        <svg ref={topologySvg} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} viewBox={`0 0 ${layout.width} ${layout.height}`} aria-label="Interactive system topology">
          <g transform={`translate(${view.x} ${view.y}) scale(${view.scale})`}>
            {layout.edges.map((item) => { const edge = snapshot.edges.find((candidate) => candidate.id === item.id); const show = edge && (edge.kind !== "local_to" || numa !== "all") && visible.has(edge.source) && visible.has(edge.target); const traced = edge ? pathContainsEdge(tracedPath, edge.source, edge.target) : false; return show ? <path key={item.id} d={item.path} className={`edge ${edge!.kind} ${traced ? "traced" : ""}`}/> : null; })}
            {layout.nodes.map((box) => { const node = nodeById.get(box.id)!; if (!visible.has(node.id)) return null; const hasChildren = (childMap.get(node.id)?.length ?? 0) > 0; return <g key={node.id} className={`node ${highlighted.has(node.id) ? "highlighted" : ""} ${selected.includes(node.id) ? "selected" : ""}`} transform={`translate(${box.x} ${box.y})`} onClick={(event) => { event.stopPropagation(); selectNode(node.id); }}>
              <rect width={box.width} height={box.height} rx="3"/><rect className="kind-bar" width="4" height={box.height} fill={colors[node.kind]}/><text x="16" y="22">{truncate(node.label, 29)}</text><text className="secondary" x="16" y="39">{String(node.facts.pci_bdf?.value ?? node.facts.logical_index?.value ?? node.kind.replaceAll("_", " "))}</text>{hasChildren && <g className="collapse-control" role="button" aria-label={collapsed.has(node.id) ? "Expand subtree" : "Collapse subtree"} onClick={(event) => { event.stopPropagation(); toggleCollapse(node.id); }}><title>{collapsed.has(node.id) ? "Expand subtree" : "Collapse subtree"}</title><rect x={box.width - 34} y="9" width="25" height="34" rx="2"/><text x={box.width - 21} y="31">{collapsed.has(node.id) ? "▸" : "▾"}</text></g>}
            </g>; })}
          </g>
        </svg>
        <div className="hint">SCROLL TO ZOOM · DRAG TO PAN · USE NODE +/− TO COLLAPSE · SELECT A THEN B TO TRACE</div>
      </section>
      <aside className="details">{selectedNode ? <Details node={selectedNode} snapshot={snapshot} collapsed={collapsed.has(selectedNode.id)} descendantCount={countDescendants(selectedNode.id, childMap)} onToggleCollapse={() => toggleCollapse(selectedNode.id)}/> : <EmptyDetails snapshot={snapshot}/>}</aside>
    </main>
  </div>;
}

function Details({ node, snapshot, collapsed, descendantCount, onToggleCollapse }: { node: TopologyNode; snapshot: TopologySnapshot; collapsed: boolean; descendantCount: number; onToggleCollapse: () => void }) {
  ...
  return <><div className="details-heading"><span className="kind-dot" style={{ background: colors[node.kind] }}/><div><label>{node.kind.replaceAll("_", " ")}</label><h2>{node.label}</h2><code>{node.id}</code></div></div>{descendantCount > 0 && <button className="subtree-action" onClick={onToggleCollapse}>{collapsed ? "▸ Expand subtree" : `▾ Collapse subtree · ${descendantCount} descendants`}</button>}<section><label className="section-label">FACTS & PROVENANCE</label>{Object.entries(node.facts).map(([key, fact]) => <details key={key}><summary><span>{key}</span><b className={fact.state}>{fact.value === null ? "UNKNOWN" : String(fact.value)}</b></summary>{fact.provenance.map((provenance, index) => <dl key={index}><dt>state</dt><dd>{fact.state}</dd><dt>collector</dt><dd>{provenance.collector}</dd><dt>source field</dt><dd>{provenance.sourceField}</dd><dt>raw</dt><dd>{String(provenance.rawValue)}</dd>{provenance.derivationRule && <><dt>rule</dt><dd>{provenance.derivationRule}</dd></>}</dl>)}</details>)}</section>{commands.length > 0 && <section><label className="section-label">VERIFY THIS OBJECT</label>{commands.map((item) => <div className="investigate" key={item.command}><div><b>{item.label}</b><button title="Copy command" onClick={() => navigator.clipboard?.writeText(item.command)}>COPY</button></div><code>{item.command}</code><p>{item.reason}</p></div>)}</section>}<section><label className="section-label">RELATIONSHIPS · {relationships.length}</label>{relationships.map((edge) => <div className="relationship" key={edge.id}><b>{edge.kind}</b><code>{edge.source === node.id ? `→ ${edge.target}` : `← ${edge.source}`}</code></div>)}</section></>;
}
function EmptyDetails({ snapshot }: { snapshot: TopologySnapshot }) { const count = (kind: NodeKind) => snapshot.nodes.filter((node) => node.kind === kind).length; const memory = snapshot.nodes.filter((node) => node.kind === "memory_region").reduce((sum, node) => sum + Number(node.facts.capacity_bytes?.value ?? 0), 0); const insights = topologyInsights(snapshot); return <div className="empty-details"><label className="section-label">SNAPSHOT SUMMARY</label><h2>{snapshot.nodes.find((node) => node.id === snapshot.hostId)?.label}</h2><div className="summary-grid"><span><b>{count("cpu_package")}</b>CPU packages</span><span><b>{count("numa_node")}</b>NUMA nodes</span><span><b>{count("gpu")}</b>GPU nodes</span><span><b>{count("nic")}</b>NIC nodes</span><span><b>{count("network_interface")}</b>interfaces</span><span><b>{count("rdma_device")}</b>RDMA devices</span><span><b>{formatBytes(memory)}</b>memory</span><span><b>{snapshot.diagnostics.length}</b>diagnostics</span></div><p>Select endpoint A, then endpoint B, to trace their known containment path. Select any node for exact evidence and read-only investigation commands.</p><section className="insights"><label className="section-label">ENGINEERING INSIGHTS · {insights.length}</label>{insights.map((insight) => <article key={insight.id} className={insight.severity}><b>{insight.title}</b><p>{insight.finding}</p><small>{insight.evidence}</small></article>)}</section></div>; }
...
function countDescendants(id: string, children: ReadonlyMap<string, string[]>): number { return (children.get(id) ?? []).reduce((count, child) => count + 1 + countDescendants(child, children), 0); }
