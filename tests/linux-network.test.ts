...
import { parseIpLinkJson, parseRdmaLinkJson } from "../src/adapters/iproute";
...
describe("generic Linux network enrichment", () => {
  it("correlates netdev, PCI, NUMA, RDMA device, and port using explicit keys", () => {
    ...
    const snapshot = enrichLinuxNetwork(base, {
      ...
      collectors: [collector("linux.ip_link"), collector("linux.sysfs_net"), collector("linux.rdma_link")]
    });
